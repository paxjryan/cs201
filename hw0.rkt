...
;(define (kg2lb mass)
;(let* ([lb (inexact->exact(floor(* mass 2.2)))]
;       [oz (inexact->exact(floor(* 16 (- (* mass 2.2) lb))))]
;       [output (list lb 'pounds oz 'ounces)])
;   output))

(define (kg2lb mass)
  (let* ([lb (inexact->exact(floor(* mass 2.2)))] ;set lb to the floor of mass * 2.2
        [oz (inexact->exact(floor(* 16 (- (* mass 2.2) lb))))] ;set oz to the floor of the difference in mass * 2.2 and lb, times 16
        [output (cond
                  [(and (equal? lb 1) (equal? oz 1)) (list lb 'pound oz 'ounce)] ;if lb and oz are both 1, use 'pound and 'ounce
                  [(equal? lb 1) (list lb 'pound oz 'ounces)] ;if lb only is 1, use 'pound and 'ounces
                  [(equal? oz 1) (list lb 'pounds oz 'ounce)] ;if oz only is 1, use 'pounds and 'ounce
                  [else (list lb 'pounds oz 'ounces)])]) ;otherwise, use 'pounds and 'ounces
    output))
