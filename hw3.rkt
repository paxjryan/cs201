...
; (conf 'q1 '() 1 '()) 20)
   ...
   ; q1 moves right until it finds a blank, writes an x, then moves right into q2
   (ins 'q1 0 'q1 0 'R)
   (ins 'q1 1 'q1 1 'R)
   (ins 'q1 'b 'q2 'x 'R)
   ; q2 moves left until it finds an x, then moves left into q3
   (ins 'q2 0 'q2 0 'L)
   (ins 'q2 1 'q2 1 'L)
   (ins 'q2 'b 'q2 'b 'L)
   (ins 'q2 'x 'q3 'x 'L)
   ; q3 moves left until it finds a 0, 1, or b
   ; IF 0: replace 0 with x, move right into q4
   ; IF 1: replace 1 with x, move right into q5
   ; IF b: we've reached the start of the tape, so move right into q6
   (ins 'q3 0 'q4 'x 'R)
   (ins 'q3 1 'q5 'x 'R)
   (ins 'q3 'x 'q3 'x 'L)
   (ins 'q3 'b 'q6 'b 'R)
   ; q4 moves right until it finds a blank, writes a 0, and moves left into q2
   (ins 'q4 0 'q4 0 'R)
   (ins 'q4 1 'q4 1 'R)
   (ins 'q4 'x 'q4 'x 'R)
   (ins 'q4 'b 'q2 0 'L)
   ; q5 moves right until it finds a blank, writes a 1, and moves left into q2
   (ins 'q5 0 'q5 0 'R)
   (ins 'q5 1 'q5 1 'R)
   (ins 'q5 'x 'q5 'x 'R)
   (ins 'q5 'b 'q2 1 'L)
   ; q6 moves right, replacing xs with blanks
   (ins 'q6 'x 'q6 'b 'R)
   ))
...
(define (i-match? state symbol inst)
  (and (equal? symbol (ins-c-symbol inst))
       (equal? state (ins-c-state inst))))
...
(define (normalize config)
  (cond
    ; If the first element of conf-ltape is a blank, call normalize on a new config with that blank removed
    [(and (not (empty? (conf-ltape config))) (or (equal? 'b (conf-ltape config)) (equal? 'b (car (conf-ltape config)))))
     (normalize (conf (conf-state config)
                      (cdr (conf-ltape config))
                      (conf-symbol config)
                      (conf-rtape config)))]
    ; If the last element of conf-rtape is a blank, call normalize on a new config with that blank removed
    [(and (not (empty? (conf-rtape config))) (or (equal? 'b (conf-rtape config)) (equal? 'b (last (conf-rtape config)))))
     (normalize (conf (conf-state config)
                      (conf-ltape config)
                      (conf-symbol config)
                      (drop-right (conf-rtape config) 1)))]
    ; If neither the first element of conf-ltape or the last element of conf-rtape is a blank, we're done!
    [else config]))
...
(define (shift-head-left config)
  (cond
    [(normalize (conf (conf-state config)
                      (if (empty? (conf-ltape config)) '() (drop-right (conf-ltape config) 1))
                      (if (empty? (conf-ltape config)) 'b (last (conf-ltape config)))
...
(define (shift-head-right config)
  (cond
    [(normalize (conf (conf-state config)
                      ...
                      (if (empty? (conf-rtape config)) 'b (car (conf-rtape config)))
                      (if (empty? (conf-rtape config)) '() (cdr (conf-rtape config)))))]))
...
(define (next-config mach config)
  (cond
    ...
    [(let ([inst (i-lookup (conf-state config) (conf-symbol config) mach)])
       (cond
         [(equal? (ins-dir inst) 'L) (shift-head-left (change-state (ins-n-state inst) (write-symbol (ins-n-symbol inst) config)))]
         [(equal? (ins-dir inst) 'R) (shift-head-right (change-state (ins-n-state inst) (write-symbol (ins-n-symbol inst) config)))]))]))
...
(define tm-convert
  (list
   ; q1 moves right until it finds a blank, then moves left into q2
   (ins 'q1 0 'q1 0 'R)
   (ins 'q1 1 'q1 1 'R)
   (ins 'q1 'b 'q2 'b 'L)
   ; q2 moves left until it finds a 1, then writes a 0 and moves right into q3
   ; If q2 finds a blank, we've reached the start of the list, so move right into q4
   (ins 'q2 0 'q2 0 'L)
   (ins 'q2 'x 'q2 'x 'L)
   (ins 'q2 1 'q3 0 'R)
   (ins 'q2 'b 'q4 'b 'R)
   ; q3 moves right and switches each 0 and 1 it comes across until it finds a blank, then writes an 'x and moves left into q2
   ; IF 0: write 1 and move right into q3
   ; IF 1: write 0 and move right into q3
   ; IF b: write x and move left into q2
   (ins 'q3 0 'q3 1 'R)
   (ins 'q3 1 'q3 0 'R)
   (ins 'q3 'x 'q3 'x 'R)
   (ins 'q3 'b 'q2 'x 'L)
   ; q4 moves right, overwriting all 0s with blanks, until it halts at the first x
   (ins 'q4 0 'q4 'b 'R)
   ))
...
(define tm-sort
  (list
   ; q1 moves right until it finds a 1, writes a c, and moves right into q2
   ; IF 0: move right into q1
   ; IF 1: write c, move right into q2
   ; IF b: the list is entirely 0s, so move left into q5
   (ins 'q1 0 'q1 0 'R)
   (ins 'q1 1 'q2 'c 'R)
   (ins 'q1 'b 'q5 'b 'L)
   ; q2 moves right until it finds a 0, writes of 1, and moves left into q3
   ; IF 0: write 1, move left into q3
   ; IF 1: write 1, move right into q2
   ; IF b: there are no more 0s left to replace, so move left into q4
   (ins 'q2 0 'q3 1 'L)
   (ins 'q2 1 'q2 1 'R)
   (ins 'q2 'b 'q4 'b 'L)
   ; q3 moves left until it finds a c, then moves right into q1
   (ins 'q3 0 'q3 0 'L)
   (ins 'q3 1 'q3 1 'L)
   (ins 'q3 'c 'q1 'c 'R)
   ; q4 moves left until it finds the first c, replaces it with a 1, and moves left into q5
   (ins 'q4 0 'q4 0 'L)
   (ins 'q4 1 'q4 1 'L)
   (ins 'q4 'c 'q5 1 'L)
   ; q5 moves left, replacing all cs with 0s, until it halts at a blank
   (ins 'q5 0 'q5 0 'L)
   (ins 'q5 1 'q5 1 'L)
   (ins 'q5 'c 'q5 0 'L)
   (ins 'q5 'b 'q6 'b 'R)
